...
const EmployeeDashboard = ({ data }: EmployeeDashboardProps) => {
  ...
  const chartData = [
    ...
    { month: "Aug", PresentDays: data?.currentMonthAttendance || 20, LeavesTaken: data?.pendingLeaves || 2, NetSalary: data?.latestPayslip?.netSalary || 2000 },
  ];
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              {emp?.position} • <span className="text-indigo-400 font-medium">{emp?.department}</span>
            </p>
          </div>
        </div>

        <div className="flex flex-wrap gap-3">
          <Link to="/attendance" className="btn-primary flex items-center gap-2 py-2 text-xs shadow-none shrink-0">
            <HiOutlineLogin className="w-4 h-4" />
            <span>Clock In / Out</span>
          </Link>
          <Link to="/leave" className="bg-slate-800 text-slate-200 border border-slate-700 hover:bg-slate-700 transition-all duration-200 px-4 py-2 rounded-md text-xs font-medium flex items-center gap-2 shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {cards.map((card, idx) => {
          ...
          return (
            <div key={idx} className="card card-hover p-6 flex flex-col justify-between min-h-[1400] bg-white shadow-xs">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{card.title}</p>
                  <p className="text-3xl font-bold text-slate-900 tracking-tight mt-2 tabular-nums">{card.value}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card p-6 bg-white shadow-xs lg:col-span-2 flex flex-col justify-between min-h-[350px]">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Attendance & Leave Trends</h3>
            </div>
            <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">6-Month History</span>
          </div>
          ...
          <div className="w-full h-64 text-xs font-medium">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="PresentDays" name="Days Present" stroke="#10b981" strokeWidth={2} fillOpacity={1} fill="url(#colorPresent)" />
                <Area type="monotone" dataKey="LeavesTaken" name="Leaves Taken" stroke="#f59e0b" strokeWidth={2} fillOpacity={1} fill="url(#colorLeaves)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="card p-6 bg-white shadow-xs lg:col-span-1 flex flex-col justify-between min-h-[350px]">
          <div>
            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Salary Status</h3>
            <p className="text-xs text-slate-400">Stable income tracking distributions</p>
          </div>
          ...
          <div className="w-full h-64 text-xs font-medium mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 5, left: -20, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" tickLine={false} />
                <Tooltip contentStyle={{ background: '#0f172a', borderRadius: '8px', color: '#fff', border: 'none' }} />
                <Bar dataKey="NetSalary" name="Net Payout ($)" fill="#6366f1" radius={[4, 4, 0, 0]} maxBarSize={30} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
